...
def initialize():
  ...
  global LS_EDGE_IDS
  ls_temp = traci.edge.getIDList()
  
  # We want to filter out the lanes and junctions. Which are known to
  # include the '_' char.
  for s_edge_id in ls_temp:
    if not '_' in s_edge_id:
      LS_EDGE_IDS.append(s_edge_id)
  return
# end def intialize
...
def create_vehicles(n_step):
  ...
  if (n_step % config.n_vehicle_spawn_rate == 0):
    ...
    # To travel westbound.
    traci.vehicle.add(s_vehicle_id, 'RT' + s_edge_init, depart=n_step+1, pos=-4, speed=-3, lane=-6, typeID="chevy_s10")
